...
## Project
Mairon scraper for peviitor.ro (Node.js, ESM, Jest)

## 📐 This Repo Is a Derived Scraper
This repo is a scraper for MAIRON GALATI SA (CIF 6581999) in the peviitor.ro ecosystem, derived from the Node.js reference template.
...
- **Only the page parsing logic in `scraper/index.js`** (`fetchJobsPage`, `parseApiJobs`) is Mairon-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/mairon-galati-sa-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/mairon-galati-sa-nodejs-scraper --state open`
