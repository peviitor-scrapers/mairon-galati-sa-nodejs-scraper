...
This scraper extracts job listings from Mairon Careers Romania API and imports them to peviitor.ro.

Target: https://www.ejobs.ro
...
1. **Start with brand** - We know the brand ("Mairon")
...
8. **Scrape new jobs** - Extract jobs from Mairon Careers API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Mairon Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Mairon Careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Mairon API (jobs for Romania)
    │
    ▼
...
- **Mairon Careers API**: `https://www.ejobs.ro/api/jobs/v2/search/careers-i18n` — GET with query params (country, page, size)
