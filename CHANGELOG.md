...
### Added
- Initial release
- Job scraping from the company's public eJobs.ro company page (https://www.ejobs.ro/company/mairon-galati-s-a/19073)
- ANOFM fallback by CIF
- Company validation via ANAF (demoanaf.ro + cuiscan.ro fallback)
- GitHub Actions workflows for daily scraping and testing
- Comprehensive test suite (unit, integration, e2e, consistency)
- GitHub Pages documentation (docs/, docs/index.html)
...
- `tests/validate-mairon-jobs.js`: multi-mode validator (`--head`, `--content`, `--browser`, `--timeout`)
...
### Changed
- `validate-jobs.js` and `tests/validate-mairon-jobs.js` are now thin CLIs that delegate to `src/job-validator.js` and `solr.js` (closes #35)
- `tests/validate-mairon-jobs.js` no longer reimplements SOLR auth, query, or delete — uses `querySOLR` + `deleteJobByUrl` from `solr.js`
...
### Changed
- `index.js`, `company.js`, `demoanaf.js`, `tests/validate-mairon-jobs.js`, `docs/index.html`, `automation-testing.yml` all now read from `config/company.json` instead of hardcoded constants
...
### Added
- Initial release
- Job scraping from Mairon Careers Romania API
