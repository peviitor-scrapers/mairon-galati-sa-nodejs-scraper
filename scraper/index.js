...
const COMPANY_SLUG = scraperConfig.companySlug;
const COMPANY_ID = scraperConfig.companyId;
...
async function fetchEJobsPage() {
  const url = `${JOB_BASE}/company/${COMPANY_SLUG}/${COMPANY_ID}`;
  ...
  if (!res.ok) {
    throw new Error(`eJobs error ${res.status} for company page`);
  }

  return await res.text();
}

function parseEJobsJobs(html) {
  const jobs = [];
  const seenUrls = new Set();

  const titlePattern = /<h2 class="job-card-content-middle__title"><a href="([^"]+)" class=""><span>([^<]+)<\/span><\/a><\/h2>/g;

  const matches = [...html.matchAll(titlePattern)];
  for (let i = 0; i < matches.length; i++) {
    const [, href, title] = matches[i];
    ...
    const locationMatch = chunk.match(/<div class="job-card-content-middle__info">([^<]*)</);
...
async function scrapeAllListings(testOnlyOnePage = false) {
  console.log("Fetching eJobs company page...");
  const html = await fetchEJobsPage();
  const jobs = parseEJobsJobs(html);

  console.log(`Found ${jobs.length} unique job(s) on the company page`);
...
async function main() {
  ...
  try {
    ...
    console.log(`Jobs scraped from eJobs company page: ${scrapedCount}`);
    ...
    console.log(`Jobs scraped from eJobs website: ${scrapedCount}`);
...
export { parseEJobsJobs, mapToJobModel, transformJobsForSOLR };
