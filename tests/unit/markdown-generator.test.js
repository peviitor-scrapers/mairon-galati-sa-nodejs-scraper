...
const baseCompany = {
  ...
  career: ["https://www.ejobs.ro"],
  lastScraped: "2026-06-17"
};

const baseJob = {
  url: "https://www.ejobs.ro/en/vacancy/123_en",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# MAIRON GALATI SA");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("6581999");
    });

    it("includes brand", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("Mairon");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.mairon.ro](https://www.mairon.ro)");
    });

    it("includes career page as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.ejobs.ro](https://www.ejobs.ro)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "6581999", company: "MAIRON GALATI SA" };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://www.ejobs.ro/en/vacancy/123_en]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://www.ejobs.ro/en/vacancy/456_en" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://www.ejobs.ro/en/vacancy/999_en", title: "QA Engineer" };
